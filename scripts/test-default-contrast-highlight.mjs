...
assert.match(consoleApp, /function applyTheme\(theme, options = \{\}\)[\s\S]*?requestAnimationFrame\(\(\) => window\.__applyReadabilityGuard\?\.\(document\.body\)\)/, "theme changes must immediately recalculate text contrast for the new surface palette");
