...
assert.match(consoleCss, /\.consulting-system \.sc-card:is\(:hover, :focus-visible, :focus-within\)[\s\S]*?background:\s*var\(--panel\);[\s\S]*?color:\s*var\(--sc-readable-ink\);/, "consulting strategy cards must keep their parent canvas fixed on hover and keyboard focus");
...
assert.match(consoleCss, /\.sc-partner,[\s\S]*?\.sc-memory-flow>li\):is\(:hover,:focus-visible,:focus-within\)[\s\S]*?background:#102f43 !important;[\s\S]*?transform:none !important;/, "small consulting cards must invert immediately without decorative movement");
