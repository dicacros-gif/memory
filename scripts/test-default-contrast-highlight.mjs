...
assert.match(consoleApp, /class="sc-framework-steps"[\s\S]*?<b>1<\/b>[\s\S]*?<b>2<\/b>[\s\S]*?<b>3<\/b>[\s\S]*?<b>4<\/b>[\s\S]*?<b>5<\/b>/, "the five-stage framework must use single-digit 1–5 labels without leading zeroes");
...
assert.match(consoleCss, /Technology-card nested tag contrast lock[\s\S]*?\.sc-tech-card:is\(:hover, :focus-visible, :focus-within\) \.sc-tech-meta > :is\(span, b\)[\s\S]*?background:\s*#f4f6f7 !important;[\s\S]*?color:\s*#10243a !important;/, "technology-card metadata must keep dark ink on its pale surface during inversion");
