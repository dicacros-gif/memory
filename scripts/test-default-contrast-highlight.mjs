...
assert.match(consoleCss, /#strategyConsulting \.sc-consulting-report \{[\s\S]*?border-radius:\s*0 !important;[\s\S]*?background:\s*linear-gradient\(180deg,[\s\S]*?box-shadow:\s*0 14px 34px/, "account reports must use a square-cornered paper-and-rail consulting frame");
assert.match(consoleCss, /details\.sc-consulting-report > summary\.sc-report-head > strong > b \{[\s\S]*?width:\s*34px;[\s\S]*?height:\s*34px;[\s\S]*?padding:\s*0;[\s\S]*?border-radius:\s*50%;/, "account report indexes must render as true circles");
...
assert.match(consoleCss, /\.projection-scenario-tab \{[\s\S]*?border-radius:\s*0;[\s\S]*?linear-gradient\(135deg,[\s\S]*?#0b1e2c[\s\S]*?cursor:\s*pointer;/, "projection scenario buttons must use a square dark-gradient surface");
...
assert.match(consoleCss, /\.qa-strategy-output \{[\s\S]*?--answer-accent:\s*#e07a12;[\s\S]*?border-color:\s*#e07a12;[\s\S]*?border-top:\s*4px solid #e07a12;/, "the QA output rail must keep its orange border in default and interactive states");
assert.match(consoleCss, /\.sc-consulting-report \.sc-partner \{[\s\S]*?border-top:\s*3px solid var\(--group-accent\) !important;[\s\S]*?border-radius:\s*11px !important;[\s\S]*?background-image:\s*linear-gradient\(155deg,/, "customer and partner cards must share one accent-led paper-gradient system");
