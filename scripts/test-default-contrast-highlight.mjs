...
assert.ok(contrastRatio("#cfbd9d", "#081e2e") >= 3, "sidebar focus ring must remain distinguishable on the navy rail");
...
assert.match(css, /\.business-pain-framework \.business-framework-panel dt[\s\S]*?color:\s*#15756F/);
...
assert.match(css, /Sparse decision emphasis[\s\S]*?mark\.business-key-term[\s\S]*?color:\s*inherit !important[\s\S]*?background:\s*transparent !important[\s\S]*?text-decoration-line:\s*underline[\s\S]*?text-decoration-color:\s*#bd9855/);
assert.doesNotMatch(css, /mark\.business-key-term[\s\S]{0,500}#ded6c6/, "decision terms must not use a filled yellow marker");
...
assert.match(css, /\.business-site \.business-competency-card mark\.business-key-term \{[\s\S]*?background:\s*transparent !important;[\s\S]*?text-decoration-color:\s*#d5cbb8 !important;/, "capability key terms must use a restrained yellow underline without a filled marker");
assert.match(css, /Automation status is always legible[\s\S]*?\.business-data-status \{[\s\S]*?color:\s*#f7fbff;[\s\S]*?background:[\s\S]*?#071521;[\s\S]*?\.business-data-status\.business-reveal \{[\s\S]*?opacity:\s*1;/, "automation text must remain visible before reveal animation completes");
...
assert.match(css, /\.business-rag-operating-model\[data-hover-mode="dark-to-light"\]:is\(:hover, :focus-within\) \.business-rag-maturity b \{[\s\S]*?color:\s*#071521 !important;[\s\S]*?background:\s*#a7d3d1 !important;/, "RAG maturity level badges must retain their own readable contrast");
assert.match(css, /\.business-kpi-tree\[data-hover-mode="light-to-dark"\]:is\(:hover, :focus-within\) article,[\s\S]*?\.business-partner-map\[data-hover-mode="light-to-dark"\]:is\(:hover, :focus-within\) li[\s\S]*?background:\s*#183852 !important;/, "nested KPI and partner panels must darken with a light-to-dark parent");
...
assert.match(consoleCss, /\.consulting-system \.sc-card \{[\s\S]*?--sc-readable-ink:\s*#122736;[\s\S]*?--sc-hover-surface:\s*color-mix\(in srgb, var\(--sc-accent, #447ba6\) 24%, #102b3e\);/, "consulting strategy cards must derive varied, readable inverted palettes from each card accent");
...
assert.match(mbbCss, /Hover inversion lock:[\s\S]*?\.mbb-section :is\([\s\S]*?\.mbb-record[\s\S]*?:is\(:hover, :focus-within\)[\s\S]*?background: #f3f1eb !important;[\s\S]*?-webkit-text-fill-color: #102536 !important;/, "MBB paper inversion must override cached white-text contrast classes");
...
assert.doesNotMatch(consoleApp, /class="sc-framework-steps sc-framework-steps-seven"[\s\S]{0,2200}<li tabindex="0">/, "informational strategy stages must not create inert keyboard stops");
...
assert.match(consoleApp, /claimLabel: lens \? "전략 가설" : "전략 가설·직접 근거 미확보"[\s\S]*?claimLabel: "검증 조건"/, "proposed business choices and gates must be labelled as hypotheses and validation conditions");
assert.match(consoleApp, /const isResearchEvidence = hasEvidence[\s\S]*?const isOfficialEvidence = hasEvidence && !isResearchEvidence[\s\S]*?const evidenceTierLabel = isOfficialEvidence \? "공식 근거" : isResearchEvidence \? "리서치 근거" : "직접 근거 미확보"/, "account evidence must distinguish official, research and unverified tiers");
...
assert.match(consoleCss, /\.sc-account-stage \.sc-claim-label \{[\s\S]*?flex:\s*0 1 auto;[\s\S]*?max-width:\s*100%;[\s\S]*?overflow-wrap:\s*anywhere;[\s\S]*?white-space:\s*normal;/, "long account evidence labels must wrap inside narrow decision columns");
...
assert.match(consoleApp, /class="exec-baseline-level-index contrast-surface">L\$\{String\(index \+ 1\)\}<\/span>/, "broker cards must render a boxed level index instead of a circular number");
...
assert.match(consoleCss, /Memory-bypass readability lock[\s\S]*?--mbp-readable-ink:\s*#163349;[\s\S]*?--mbp-readable-muted:\s*#415769;[\s\S]*?\.mbp-route-copy strong[\s\S]*?color:\s*var\(--mbp-readable-ink\) !important;[\s\S]*?\.mbp-tl-items li[\s\S]*?color:\s*var\(--mbp-readable-muted\) !important;/, "memory bypass route and timeline copy must stay legible on light cards despite cached contrast classes");
assert.match(consoleCss, /Council agenda contrast lock[\s\S]*?--agenda-surface:\s*#ffffff;[\s\S]*?--agenda-ink:\s*#163349;[\s\S]*?--agenda-muted:\s*#3e5566;/, "council agendas must define an explicit readable palette on light surfaces");
...
assert.match(consoleCss, /\.ai-council-agenda-card:is\(:hover, :focus-visible\)[\s\S]*?--agenda-surface:\s*#183852;[\s\S]*?--agenda-ink:\s*#f7fbff;[\s\S]*?\[data-theme="dark"\][\s\S]*?--agenda-surface:\s*#f8fafc;[\s\S]*?--agenda-ink:\s*#122736;/, "council agendas must invert surface and copy together in both themes");
...
assert.match(consoleCss, /Equity value-chain readability lock[\s\S]*?--equity-readable-ink:\s*#163349;[\s\S]*?--equity-readable-muted:\s*#3e5566;[\s\S]*?--equity-readable-accent:\s*#15756F;/, "the listed value-chain must define an explicit light-surface palette");
...
assert.match(consoleCss, /\.equity-chain-lane button:is\(:hover, :focus-visible\)[\s\S]*?--equity-item-ink:\s*#f7fbff;[\s\S]*?background:\s*#183852 !important;[\s\S]*?\[data-theme="dark"\][\s\S]*?--equity-item-ink:\s*#122736;[\s\S]*?background:\s*#f8fafc !important;/, "value-chain buttons must invert surface and copy together in both themes");
...
assert.match(consoleCss, /#hyperscaler-demand \.hs-card \.hs-signal\.insufficient\s*\{[\s\S]*?-webkit-text-fill-color:\s*#32495a\s*!important;/, "light evidence pills must lock the visible glyph fill even when their parent was corrected for a dark surface");
...
assert.match(landing, /const nearest = element\.closest\(INTERACTION_SCOPE\) \|\| element;[\s\S]*?nearest\.closest\("\.qa-dropdown, \.answer-panel"\)[\s\S]*?return;[\s\S]*?scheduleAudit\(surface\)/, "QA hover and focus inversion must not wait for a computed-style readability audit");
...
assert.match(consoleCss, /\.strategy-highlight, \.answer-term\):not\(\.ui-key-term\)[\s\S]*?box-shadow:\s*none !important;[\s\S]*?\.strategy-highlight, \.answer-term\)\.ui-key-term[\s\S]*?inset 0 -2px 0 #bd9855/, "only selected Console terms may receive the amber underline");
assert.match(consoleCss, /\.decision-card, \.decision-flip-card, \.domain-agent-workstream[\s\S]*?--console-hover-surface:\s*#102b3e;[\s\S]*?\[data-theme="dark"\][\s\S]*?--console-hover-surface:\s*#f8fafc;/, "Console decision cards must invert legibly in light and dark modes");
assert.match(consoleCss, /Memory-bypass index contrast lock[\s\S]*?\.mbp-thesis-arrow, \.mbp-band-n, \.mbp-tl-marker[\s\S]*?color:\s*#163349 !important;[\s\S]*?\.mbp-route-n[\s\S]*?color:\s*#ffffff !important;[\s\S]*?background:\s*#163349 !important;/, "memory-tier arrows and level indices must retain readable ink over accent surfaces");
assert.match(consoleCss, /Progressive market modules[\s\S]*?\.equity-company-monogram[\s\S]*?color:\s*#163349 !important;[\s\S]*?\.price-sub[\s\S]*?color:\s*#495f6f !important;[\s\S]*?data-theme="dark"[\s\S]*?color:\s*#d7e2ee !important;/, "delayed market modules must use mode-safe text colors");
assert.match(consoleCss, /Instant card inversion and contrast lock[\s\S]*?--console-surface:\s*#f8fafc;[\s\S]*?--console-ink:\s*#122736;[\s\S]*?--console-surface-inverse:\s*#102b3e;[\s\S]*?--console-ink-inverse:\s*#f8fbff;[\s\S]*?\[data-theme="dark"\] #intelligenceConsole[\s\S]*?--console-surface:\s*#102b3e;[\s\S]*?--console-surface-inverse:\s*#f8fafc;/, "Console inversion must swap paired surface and ink tokens in both themes");
...
assert.match(consoleCss, /Nested news-card contrast lock[\s\S]*?\.consulting-system #intelligenceConsole \.news-card:is\(:hover, :focus-visible, :focus-within\) \.news-insights > li[\s\S]*?background:\s*rgba\(248, 250, 252, \.97\) !important;[\s\S]*?color:\s*#304350 !important;[\s\S]*?> li::before[\s\S]*?color:\s*#15756F !important;/, "nested news bullet rows must retain dark readable ink on light hover surfaces");
assert.match(consoleCss, /Technology-card nested tag contrast lock[\s\S]*?#strategyConsulting \.sc-tech-card \.sc-tech-meta > :is\(span, b\)[\s\S]*?background:\s*#f4f6f7 !important;[\s\S]*?color:\s*#0f2435 !important;/, "technology-card metadata must keep dark ink on its pale surface in default and inverted states");
assert.match(consoleCss, /Evidence-report row contrast lock[\s\S]*?\.sc-evidence-list li:is\(:hover, :focus-within\)[\s\S]*?background:\s*#f4f7f9 !important;[\s\S]*?\.sc-ev-src[\s\S]*?color:\s*#15756F !important;[\s\S]*?li:is\(:hover, :focus-within\) a[\s\S]*?color:\s*#102c43 !important;/, "evidence rows must invert surface, source, link and date as one readable palette");
assert.match(consoleCss, /Nested paper-surface contrast locks[\s\S]*?\.share-card \.share-metrics > div[\s\S]*?background:\s*#f4f7f9 !important;[\s\S]*?color:\s*#0f2435 !important;/, "competitor metric tiles must retain dark ink on their paper surfaces");
assert.match(consoleCss, /Nested paper-surface contrast locks[\s\S]*?\.company-strategy-flow > a,[\s\S]*?\.company-focus-card li[\s\S]*?background:\s*#f6f7fb !important;[\s\S]*?color:\s*#0f2435 !important;/, "company strategy and decision rows must preserve the paper-surface ink pair");
assert.match(consoleCss, /Nested paper-surface contrast locks[\s\S]*?\.company-evidence-meta i[\s\S]*?background:\s*#e9f5ff !important;[\s\S]*?color:\s*#162b3c !important;/, "company evidence pills must use dark ink on lavender paper");
...
assert.match(consoleCss, /\.projection-scenario-tab::after \{[\s\S]*?z-index:\s*2;[\s\S]*?background:\s*#f8fbfc;[\s\S]*?color:\s*#0f2638;[\s\S]*?-webkit-text-fill-color:\s*#0f2638;[\s\S]*?transition:\s*none;/, "projection scenario action labels must remain visible without delayed colour transitions");
assert.match(consoleCss, /\.projection-scenario-tab\.active::after \{[\s\S]*?background:\s*color-mix\(in srgb, var\(--scenario-accent\) 72%, #071825\);[\s\S]*?color:\s*#fff;[\s\S]*?-webkit-text-fill-color:\s*#fff;/, "selected scenario labels must keep white text on a contrast-safe accent surface");
...
assert.match(consoleCss, /\.qa-strategy-output \{[\s\S]*?--answer-accent:\s*#b58e46;[\s\S]*?border-color:\s*#b58e46;[\s\S]*?border-top:\s*4px solid #b58e46;/, "the QA output rail must keep its orange border in default and interactive states");
...
assert.match(consoleCss, /MBB executive visual system[\s\S]*?--mbb-ink:#112e45;[\s\S]*?--info-cyan:#94cac7;/, "the Console must expose one restrained MBB palette for every executive module");
...
assert.doesNotMatch(consoleCss, /:is\(mark[^{]*\{[^}]*background:\s*#d9d0bf/, "the marker-pen wash must stay deleted from key terms");
assert.match(consoleCss, /\.ni-research-list li\.ni-cite:is\(:hover, :focus-within\) \.ni-cite-src[\s\S]*?background:\s*#dff8f2 !important;[\s\S]*?color:\s*#0b3c38 !important;/, "publisher pills must keep dark readable text on their pale surface during inversion");
assert.match(consoleCss, /\.ni-cite \.crawl-remove-button:is\(:hover, :focus-visible\)[\s\S]*?background:\s*#d9d0bf !important;[\s\S]*?color:\s*#16120a !important;/, "citation close controls must remain readable on hover and keyboard focus");
...
assert.match(consoleCss, /Advanced decision module contrast lock[\s\S]*?\.advanced-module-card:is\(:hover, :focus-visible, :focus-within\) \.advanced-score[\s\S]*?background:\s*#f8fafc !important;[\s\S]*?color:\s*#152835 !important;/, "advanced scorecards must keep dark ink on paper during parent-card inversion");
assert.match(consoleCss, /\.advanced-module-card:is\(:hover, :focus-visible, :focus-within\) \.advanced-score strong[\s\S]*?color:\s*#125268 !important;[\s\S]*?\.advanced-module-card:is\(:hover, :focus-visible, :focus-within\) \.advanced-score small[\s\S]*?color:\s*#314757 !important;/, "advanced score values and notes must remain visible in the inverted state");
...
assert.match(consoleCss, /\.talent-roi-story[\s\S]*?--console-surface-inverse:\s*#102b3e;[\s\S]*?--console-ink-inverse:\s*#f8fbff;/, "image-backed talent stories must retain a dark readable hover surface");
...
assert.match(consoleCss, /Company fact-row contrast lock[\s\S]*?--company-fact-surface:\s*#eaf1f5;[\s\S]*?--company-fact-ink:\s*#163349;[\s\S]*?\.company-fact-card li span[\s\S]*?color:\s*var\(--company-fact-ink\) !important;/, "company fact rows must keep dark readable copy on their pale surface");
assert.match(consoleCss, /\.company-fact-card li:is\(:hover, :focus-within\)[\s\S]*?--company-fact-surface:\s*#183852;[\s\S]*?--company-fact-ink:\s*#f7fbff;[\s\S]*?--company-fact-label:\s*#bdd9d7;/, "company fact rows must invert their surface, value, and label colors together");
...
assert.match(mbbCss, /\.mbb-frame\[data-frame="case-board"\][\s\S]*?partnership-qualification-lab-bg\.webp[\s\S]*?\.mbb-frame\[data-frame="case-board"\] \.mbb-case \{[\s\S]*?background:\s*rgba\(8, 43, 56, \.88\);/, "the partnership case board must use its own dark validation-lab background and readable translucent cards");
