...
assert.match(consoleCss, /Strategy consulting frame system[\s\S]*?\.sc-report,[\s\S]*?border-radius:\s*0 !important;[\s\S]*?box-shadow:\s*5px 5px 0 var\(--sc-offset\) !important;/, "strategy sections must use square full-perimeter consulting frames instead of rounded AI cards");
assert.match(consoleCss, /\.sc-partner,[\s\S]*?\.sc-hyperscaler-node,[\s\S]*?border:1px solid var\(--sc-frame\) !important;[\s\S]*?border-radius:0 !important;[\s\S]*?box-shadow:4px 4px 0 var\(--sc-offset\) !important;/, "customer, technology, and partner cards must share the full-frame offset-shadow system");
assert.match(consoleCss, /\.sc-partner,[\s\S]*?\.sc-memory-flow>li\):is\(:hover,:focus-visible,:focus-within\)[\s\S]*?background:#102f43 !important;[\s\S]*?transform:translate\(-1px,-1px\) !important;/, "small consulting cards must invert immediately without moving the enclosing board");
