...
assert.match(consoleCss, /\.sc-framework-steps > li::before \{[\s\S]*?border-top:\s*3px solid var\(--stage-accent\);[\s\S]*?clip-path:\s*polygon\(/, "the executive framework must use angled consulting cards with stage-specific rails");
assert.match(consoleCss, /\.sc-framework-steps > li:is\(:hover, :focus-visible\)::before \{[\s\S]*?background:\s*var\(--stage-accent\);/, "each consulting stage must invert its own surface on hover or keyboard focus");
assert.match(consoleCss, /\.sc-framework-steps > li:is\(:hover, :focus-visible\) b \{[\s\S]*?background:\s*#fff;[\s\S]*?color:\s*var\(--stage-accent\);/, "number badges must remain readable on the inverted stage card");
assert.match(consoleCss, /transition:\s*background-color 0s, border-color 0s;/, "stage-card color inversion must occur without delay");
...
assert.match(consoleCss, /\.sc-card-flow \.sc-map-node \{[\s\S]*?clip-path:polygon\(0 0,calc\(100% - 10px\) 0,100% 50%/, "the opportunity flow must use connected consulting chevrons instead of generic rounded cards");
...
assert.match(consoleCss, /@keyframes mbb-flow-pulse[\s\S]*?opacity:\.38;[\s\S]*?opacity:1;/, "diagram connectors must use a restrained signal-flow animation");
