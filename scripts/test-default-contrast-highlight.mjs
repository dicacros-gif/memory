...
assert.match(consoleCss, /#intelligenceConsole :is\(\.sc-card,[\s\S]*?background:\s*var\(--mbb-paper\) !important;/, "consulting strategy cards must keep their parent canvas fixed in the MBB paper system");
...
assert.match(consoleCss, /#intelligenceConsole :is\(\.sc-framework-steps>li,[\s\S]*?\.ai-council-signal-chain>div\) \{[\s\S]*?background:var\(--mbb-step\) !important;[\s\S]*?clip-path:polygon\(/, "multi-stage diagrams must share one continuous MBB ribbon surface");
assert.match(consoleCss, /#intelligenceConsole :is\(\.sc-framework-steps>li,[\s\S]*?\.ai-council-signal-chain>div\):is\(:hover,:focus-visible,:focus-within\) \{[\s\S]*?background:#f7fafb !important;/, "each ribbon stage must invert independently on hover or keyboard focus");
assert.match(consoleCss, /#intelligenceConsole \.sc-framework-steps>li>b,[\s\S]*?\.sc-radar-layers>li\)>b \{[\s\S]*?border-radius:0;/, "stage indices must use flat consulting labels rather than circular AI badges");
assert.match(consoleCss, /transition:background-color 0s,color 0s,border-color 0s !important;/, "stage-card color inversion must occur without delay");
...
assert.match(consoleApp, /class="sc-map-node sc-workload" data-step="01" tabindex="0"[\s\S]*?class="sc-map-node sc-gate" data-step="05" tabindex="0"/, "every decision box must expose its independent inversion to keyboard users");
...
assert.match(consoleCss, /#intelligenceConsole :is\(\.sc-framework-steps,\.sc-card-flow,\.sc-memory-flow,\.sc-radar-layers,\.visual-insight-route,\.ai-council-signal-chain\) \{[\s\S]*?gap:0 !important;/, "all multi-stage maps must read as continuous consulting ribbons");
...
assert.match(consoleCss, /Nested news-card contrast lock[\s\S]*?\.consulting-system #intelligenceConsole \.news-card:is\(:hover, :focus-visible, :focus-within\) \.news-insights > span[\s\S]*?background:\s*rgba\(248, 250, 252, \.97\) !important;[\s\S]*?color:\s*#334155 !important;[\s\S]*?> b[\s\S]*?color:\s*#08766f !important;/, "nested news evidence rows must retain dark readable ink on light hover surfaces");
assert.match(consoleCss, /Professional consulting matrix — square, flat, neutral hierarchy\.[\s\S]*?\.sc-report,[\s\S]*?border-radius:0 !important;[\s\S]*?box-shadow:none !important;/, "strategy sections must use square full-perimeter consulting frames instead of rounded AI cards");
assert.match(consoleCss, /\.sc-partner,[\s\S]*?\.sc-hyperscaler-node,[\s\S]*?border:1px solid var\(--sc-frame\) !important;[\s\S]*?border-radius:0 !important;[\s\S]*?box-shadow:none !important;/, "customer, technology, and partner cards must share the same flat full-frame system");
assert.match(consoleCss, /\.sc-partner,[\s\S]*?\.sc-memory-flow>li\):is\(:hover,:focus-visible,:focus-within\)[\s\S]*?background:var\(--mbb-local\) !important;[\s\S]*?transform:none !important;/, "small consulting cards must invert immediately without decorative movement");
assert.match(consoleCss, /MBB executive visual system[\s\S]*?--mbb-ink:#102f43;[\s\S]*?--mbb-teal:#08766f;[\s\S]*?--mbb-amber:#9b6200;/, "the Console must expose one restrained MBB palette for every executive module");
assert.match(consoleCss, /\.sc-card-flow>\.sc-map-node,[\s\S]*?clip-path:polygon\(0 0,calc\(100% - 16px\) 0,100% 50%/, "the opportunity flow must use connected consulting chevrons instead of generic rounded cards");
