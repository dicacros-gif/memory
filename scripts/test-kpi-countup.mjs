...
assert.match(html, /class="memory-video-hero"[\s\S]*?id="memoryHeroVideo"[^>]*preload="none"[^>]*poster="assets\/media\/memory-hero-poster\.webp"/, "the console must restore its background video behind a persistent opening poster");
assert.match(html, /data-src="assets\/media\/memory-hero-lite\.mp4"/, "the opening video must retain its compact fallback");
