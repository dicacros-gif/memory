...
assert.match(css, /Architecture-track contrast lock[\s\S]*?\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.metric[\s\S]*?background: #edf3f5 !important;[\s\S]*?color: #142438 !important;/, "inverted architecture tracks must retain dark ink on pale metric surfaces");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.tag[\s\S]*?background: #f8fafc !important;[\s\S]*?color: #24364a !important;/, "inverted architecture tracks must retain dark ink on pale tags");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.tag :is\(strong, span, small, b, em, a, \.strategy-highlight, \.company-profile-link\)[\s\S]*?color: #24364a !important;[\s\S]*?-webkit-text-fill-color: #24364a !important;/, "nested links and semantic highlights inside pale tags must not inherit the inverted card's white ink");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.baseline-freshness[\s\S]*?background: #e3f7f3 !important;[\s\S]*?color: #0d4f4a !important;/, "inverted architecture tracks must retain dark ink on the freshness pill");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.baseline-freshness :is\(strong, span, small, b, em, a, \.strategy-highlight, \.company-profile-link\)[\s\S]*?color: #0d4f4a !important;/, "freshness-pill links must retain teal-black ink during parent-card inversion");
