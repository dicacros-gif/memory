...
assert.match(css, /--console-surface:\s*#f8fafc;[\s\S]*?--console-ink:\s*#122736;[\s\S]*?--console-surface-inverse:\s*#102b3e;[\s\S]*?--console-ink-inverse:\s*#f8fbff;/, "card inversion must define paired light-surface and dark-surface tokens");
assert.match(css, /\[data-theme="dark"\] #intelligenceConsole,[\s\S]*?\{[\s\S]*?--console-surface:\s*#102b3e;[\s\S]*?--console-ink:\s*#f8fbff;[\s\S]*?--console-surface-inverse:\s*#f8fafc;[\s\S]*?--console-ink-inverse:\s*#122736;/, "dark mode must swap the paired surface and ink tokens together");
...
assert.match(html, /class="memory-video-hero"[\s\S]*?id="memoryHeroVideo"[^>]*preload="none"[^>]*poster="assets\/media\/investor-equity-hero-poster\.webp"/, "the console must use a neutral investor video behind a persistent opening poster");
assert.match(html, /data-src="assets\/media\/investor-equity-hero\.mp4"/, "the opening video must use the compact neutral asset");
assert.match(landing, /function setupConsoleHeroExperience\([\s\S]*?source\.src = source\.dataset\.src[\s\S]*?requestIdleCallback/, "the opening video must hydrate after the Console shell instead of blocking it");
...
assert.match(css, /\.market-peer-card \{[\s\S]*?--peer-frame: color-mix\(in srgb, #657987 56%, var\(--line\)\);[\s\S]*?border: 2px solid var\(--peer-frame\);[\s\S]*?linear-gradient\(145deg,[\s\S]*?#607686 7%/, "peer cards should use a restrained shared slate frame");
...
assert.match(css, /Architecture-track contrast lock[\s\S]*?\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.metric[\s\S]*?background: #edf3f5 !important;[\s\S]*?color: #122534 !important;/, "inverted architecture tracks must retain dark ink on pale metric surfaces");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.tag[\s\S]*?background: #f8fafc !important;[\s\S]*?color: #223746 !important;/, "inverted architecture tracks must retain dark ink on pale tags");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.tag :is\(strong, span, small, b, em, a, \.strategy-highlight, \.company-profile-link\)[\s\S]*?color: #223746 !important;[\s\S]*?-webkit-text-fill-color: #223746 !important;/, "nested links and semantic highlights inside pale tags must not inherit the inverted card's white ink");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.baseline-freshness[\s\S]*?background: #e3f7f3 !important;[\s\S]*?color: #0e4f4a !important;/, "inverted architecture tracks must retain dark ink on the freshness pill");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\) \.baseline-freshness :is\(strong, span, small, b, em, a, \.strategy-highlight, \.company-profile-link\)[\s\S]*?color: #0e4f4a !important;/, "freshness-pill links must retain teal-black ink during parent-card inversion");
assert.doesNotMatch(css, /#strategyConsulting \.arch-track-card \.metric/, "architecture-track contrast must not depend on the unrelated strategy-consulting section");
assert.match(css, /\.arch-track-card:is\(:hover, :focus-visible, :focus-within\)[\s\S]*?:is\(h3, p, \.watch-list li\)[\s\S]*?color: #f4f9fc !important;/, "inverted architecture tracks must keep body and watch-list copy readable");
...
assert.match(css, /\.management-strategy-board \{[\s\S]*?border-color: color-mix\(in srgb, #516b80 62%, var\(--line\)\);[\s\S]*?#dce7ef[\s\S]*?#eaeff3/, "China strategy board should use a restrained slate background and outline");
assert.match(css, /\.management-strategy-board \.china-business-brief-card \{[\s\S]*?border-color: color-mix\(in srgb, #5e7b91 68%, var\(--line\)\);[\s\S]*?linear-gradient\(145deg,[\s\S]*?#edf2f6/, "China strategy briefs should use a professional white-to-slate surface");
assert.match(css, /#china-talent-strategy \.policy-card,[\s\S]*?#china-talent-strategy \.policy-rule-card,[\s\S]*?#china-talent-strategy \.policy-focus \{[\s\S]*?--talent-card-frame: #5E7283;[\s\S]*?border: 2px solid color-mix\(in srgb, var\(--talent-card-frame\) 68%, var\(--line\)\);/, "China talent cards should use one restrained slate outline");
assert.match(css, /#talentScenarioTabs \.talent-scenario-tab\.active \{[\s\S]*?linear-gradient\(135deg, #124266 0%, #29728D 54%, #15756F 100%\)/, "the selected China talent scenario should use the same professional gradient");
...
assert.match(css, /\.share-card:hover,[\s\S]*?linear-gradient\(135deg,\s*#234761,\s*#162f43\)/, "Benchmark hover should use the professional navy treatment");
...
assert.match(css, /\.exec-layout[\s\S]*?linear-gradient\(135deg,\s*#061521/, "Executive infographic should use the consulting canvas background");
...
assert.match(css, /\.agent-debate-title \.agent-tts-toggle\.needs-gesture \{[\s\S]*?#c0aa80/, "the TTS replay state should be visually distinct");
...
assert.match(css, /\.investment-focus-flow::before \{[\s\S]*?linear-gradient\(180deg, #15756F 0%, #29728D 33%, #2b5f88 67%, #2a5f88 100%\)/, "investment detail should expose a professional gradient decision path");
assert.match(css, /\.investment-focus-block\.is-priority \{ --focus-card-accent: #15756F; \}[\s\S]*?\.investment-focus-block\.is-sources \{ --focus-card-accent: #445665; \}/, "remaining investment detail blocks should use a restrained consulting palette");
...
assert.match(app, /const NUMBER_DECISION_BLUEPRINT = \[[\s\S]*?label: "QUESTION"[\s\S]*?label: "DIAGNOSIS"[\s\S]*?label: "MEMORY OPTION"[\s\S]*?label: "BUSINESS CASE"[\s\S]*?label: "DECISION"/, "number analysis should retain the complete question-to-decision structure");
...
assert.match(app, /function numberDecisionItems\(items = \[\]\)[\s\S]*?const aliases = \[entry\.title, \.\.\.\(entry\.aliases \|\| \[\]\)\][\s\S]*?const resolved = item \|\| \{[\s\S]*?fallbackValue/, "number analysis should resolve renamed metrics through aliases and retain framework fallbacks");
...
assert.match(css, /\.ceo-challenge-prompt \{[\s\S]*?linear-gradient\(118deg, #061521 0%, #0b2a41 58%, #133b59 100%\)/, "CEO challenge prompt should use a high-contrast executive header");
...
assert.match(app, /const HYPERSCALER_PROJECTION_ORDER = \[[\s\S]*?"nvidia", "google", "microsoft", "aws", "meta", "openai", "anthropic", "oracle", "apple", "tesla", "spacexai"/, "projection should preserve the governed distinct-owner account order");
...
assert.match(css, /#projection \.projection-account-tab:is\(:hover, :focus-visible, \.active\) \{[\s\S]*?--surface: #102b3e;[\s\S]*?--ink: #fff;[\s\S]*?--sub-ink: #d9e6ed;/, "hyperscaler account controls should invert with readable text tokens");
...
assert.match(css, /@media \(max-width: 1100px\) \{[\s\S]*?#projection \.projection-layout \{ grid-template-columns: minmax\(0, 1fr\); \}/, "projection detail must stack before its fixed side panel can crush account cards");
assert.match(css, /@media \(max-width: 1100px\) \{[\s\S]*?#projection \.projection-account-map \{ grid-template-columns: 1fr; \}/, "projection account map must stack every node on compact screens");
assert.match(css, /#projection \.projection-account-node :is\(span, strong, small\) \{[\s\S]*?white-space: normal;[\s\S]*?word-break: keep-all;/, "projection account copy must wrap without breaking Korean text into vertical glyphs");
...
assert.match(app, /const order = \["nvidia", "google", "microsoft", "aws", "meta", "openai", "anthropic", "oracle", "apple", "tesla", "spacexai"\]/, "forecast should organize demand by distinct named big-tech economic owners");
assert.doesNotMatch(app, /signalIds\s*=\s*\{[^}]*openai\s*:\s*"oracle"/, "OpenAI demand must never be merged into Oracle through a signal alias");
...
assert.match(css, /#hyperscaler-demand \.forecast-cat-tab:is\(:hover, :focus-visible\),[\s\S]*?--account-surface: #102b3e;[\s\S]*?--account-ink: #fff;/, "hyperscaler selectors should invert surface and ink together");
...
assert.match(css, /\.hs-scenario-tabs button\.active \{[\s\S]*?background: #102b3e;[\s\S]*?translateY\(2px\)[\s\S]*?box-shadow:/, "the active demand scenario should remain visibly pressed");
...
assert.match(css, /\.hs-logic-step:nth-of-type\(1\)[\s\S]*?\.hs-logic-step:nth-of-type\(5\)[\s\S]*?--mbb-tone-2: #785615;/, "hyperscaler decision stages should use five distinct consulting gradients");
