...
assert.doesNotMatch(app, /function setupPriceBoardPreload\(\)[\s\S]*?requestIdleCallback\(start|function setupPriceBoardPreload\(\)[\s\S]*?setTimeout\(start/, "market history must not load automatically after first paint");
...
assert.match(html, /app\.js\?v=perf-20260815-01/, "JavaScript cache key should include the performance revision");
assert.match(html, /styles\.css\?v=perf-20260815-01/, "CSS cache key should include the performance revision");
