...
assert.match(html, /assets\/js\/landing\.min\.js\?v=infra-20260816-19/, "the lightweight landing controller must use the minified AI Infra revision");
...
assert.match(html, /id="automation" aria-live="polite"[\s\S]*?FAIL-CLOSED[\s\S]*?businessDataIntegrity[\s\S]*?businessDataFreshness[\s\S]*?businessDataCoverage[\s\S]*?businessDecisionConfidence[\s\S]*?SCHEDULED 3H REFRESH[\s\S]*?id="businessDataSources"[\s\S]*?id="businessDataRun"/, "the public site must separate integrity, freshness, coverage, confidence, and traceable run");
