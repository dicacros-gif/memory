...
assert.match(html, /assets\/js\/landing\.min\.js\?v=infra-20260816-07/, "the lightweight landing controller must use the minified AI Infra revision");
...
assert.match(html, /HBM4 \/ HBM4E 경쟁사 동종 지표 비교[\s\S]*?SK HYNIX[\s\S]*?SAMSUNG[\s\S]*?MICRON/, "Right to Win must compare all three suppliers on the same HBM4 metrics");
