...
export function renderAccountEcosystem({ ecosystem = {}, layerModel = {} } = {}) {
  ...
  return `<section class="sc-broadcom-board sc-partner-ecosystem" aria-labelledby="partnerEcosystemTitle"><header class="sc-broadcom-head"><div><span>${escapeHTML(ecosystem.eyebrow || "CUSTOM SILICON PARTNER ECOSYSTEM")}</span><h4 id="partnerEcosystemTitle">${escapeHTML(ecosystem.title || "Broadcom · Marvell → Big Tech / Hyperscaler")}</h4></div><p>${escapeHTML(ecosystem.description)}</p></header><div class="sc-partner-hierarchy" aria-label="ASIC Partner와 Big Tech 고객 계층">${partners.map((partner) => { const served = Array.isArray(partner.accounts) ? partner.accounts : []; const rollup = partner.rollup || {}; const pains = (rollup.topPainAxes || []).filter((item) => Number(item.mentions || 0) > 0).slice(0, 3); return `<section class="sc-partner-lane" style="--lane-accent:${escapeHTML(partner.accent || "#C75F4F")}"><article class="sc-partner sc-partner-node" style="--sc-accent:${escapeHTML(partner.accent || "#C75F4F")}"><span class="sc-tech-en">01 · ASIC PARTNER</span><strong data-company-id="${escapeHTML(partner.id)}">${escapeHTML(partner.company)}</strong><p>${escapeHTML(partner.chip)}</p><div class="sc-partner-row"><b>BUYING</b><span>${escapeHTML(join(partner.buyingCriteria || partnerCriteria))}</span></div><div class="sc-partner-row"><b>ROLL-UP</b><span>${escapeHTML(join(served.map((account) => account.company)))}</span></div>${pains.length ? `<div class="sc-partner-row"><b>PAIN SIGNAL</b><span>${escapeHTML(join(pains.map((item) => `${item.label} ${item.mentions}`)))}</span></div>` : ""}</article><div class="sc-hierarchy-arrow" aria-hidden="true">↓</div><div class="sc-partner-children" aria-label="${escapeHTML(partner.company)} 연결 고객">${served.map((account) => `<article class="sc-hyperscaler-node" style="--account-accent:${escapeHTML(account.accent || partner.accent || "#4A78B5")}"><span>02 · BIG TECH / HYPERSCALER</span><strong data-company-id="${escapeHTML(account.id)}">${escapeHTML(account.company)}</strong><small>${escapeHTML(account.chip)}</small><dl><div><dt>MEMORY PAIN</dt><dd>${highlight(account.pain)}</dd></div><div><dt>SKH OPTION</dt><dd>${highlight(account.memory)}</dd></div><div><dt>GATE</dt><dd>${escapeHTML(account.gate)}</dd></div></dl></article>`).join("")}</div></section>`; }).join("")}</div><div class="sc-hierarchy-criteria"><b>HYPERSCALER BUYING CRITERIA</b><span>${escapeHTML(join(customerCriteria))}</span></div></section>`;
}

const dynamicsTypeMeta = {
  competition: { label: "경쟁", accent: "#c88277" },
  partnership: { label: "파트너십", accent: "#6089be" },
  investment: { label: "투자", accent: "#3bbbbb" },
  supply: { label: "공급", accent: "#d1a66d" },
  integration: { label: "플랫폼 통합", accent: "#27a8a8" },
  exploration: { label: "협력 논의", accent: "#cfb18a" },
  qualification: { label: "시스템 검증", accent: "#43c4c4" },
  adjacency: { label: "전략 유사", accent: "#a795c8" },
  hypothesis: { label: "협력 후보", accent: "#e0d3c2" },
};
...
function renderDynamicsDetail(company = {}, relations = [], companies = [], layers = []) {
  ...
  const grouped = [...new Set(connected.map((relation) => relation.type).filter(Boolean))].map((type) => ({
    type,
    ...(dynamicsTypeMeta[type] || { label: type, accent: "#9cb0c3" }),
...
export function renderCompetitiveDynamics(model = {}) {
  ...
  const evidenceGuide = `<div class="sc-dynamics-evidence" aria-label="관계선 및 근거 정책"><div class="sc-dynamics-legend">${Object.entries(dynamicsLineLabel).map(([id, label]) => `<span><i data-dynamics-line-key="${id}"></i>${label}</span>`).join("")}</div><div class="sc-dynamics-snapshot"><span><b>${relations.length}</b>검증 관계쌍</span><span><b>${Number(view.counts?.duplicateEvidence || 0)}</b>추가 공식 근거</span><span><b>${escapeHTML(shortDate(view.updatedAt) || "-")}</b>기준일</span></div><p><b>표시 기준</b>${escapeHTML(view.policy)}</p></div>`;
