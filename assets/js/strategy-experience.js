...
(() => {
  ...
  const renderAccountDetail = (profile) => {
    ...
    const flowMarkup = flow.length ? `<div class="account-flow">${flow.map((item, index) => `<article><span>${esc(text(item.index) || String(index + 1))} · ${esc(copy(item.label))}</span><strong>${esc(copy(item.value))}</strong></article>`).join("")}</div>` : "";
